...
class GlobalUpdateService {
  ...
  static GlobalUpdateService get instance => _instance ??= GlobalUpdateService._();
  ...
  static const Duration _versionCheckInterval = Duration(hours: 12); // Check twice daily
  static const Duration _quickCheckInterval = Duration(minutes: 30); // On app startup/resume
  ...
  Future<GlobalUpdateResult> checkForUpdates({bool isStartupCheck = false}) async {
    try {
      ...
      final versionSnapshot = await versionRef.get().timeout(const Duration(seconds: 3));
      ...
      final globalVersionData = Map<String, dynamic>.from(versionSnapshot.value as Map);
      ...
      // Compare versions
      final hasUpdate = _compareVersions(_currentLocalVersion, globalVersion) < 0;
  ...
  Future<void> _clearAllCaches() async {
    try {
      ...
      SongRepository.invalidateCacheForDevelopment(reason: 'Global update triggered');
  ...
  Future<void> _updateCollections() async {
    try {
      ...
      await CollectionCacheManager.instance.forceRefreshForDevelopment(
  ...
  Future<void> _loadLocalVersionInfo() async {
    try {
      ...
      if (lastCheckTimestamp != null) {
        _lastVersionCheck = DateTime.fromMillisecondsSinceEpoch(lastCheckTimestamp);
      }
      
      debugPrint('[GlobalUpdate] 📱 Local version loaded: $_currentLocalVersion');
    } catch (e) {
